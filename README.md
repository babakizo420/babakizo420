### Security Researcher | Red Team | Bug Bounty Hunter

Building offensive security tools and breaking things (with permission) under **BlessedOps**.

---

**What I do:**

- Web application security testing (OWASP Top 10, auth bypass, CSRF, IDOR)
- Web3 security research (EIP-2612 permit phishing, smart contract analysis)
- Reconnaissance automation for bug bounty programs
- NDPA/data protection compliance auditing for Nigerian businesses

**Tools I've built:**

| Project | What it does |
|---|---|
| [CyberArmor Red Team Kit](https://github.com/babakizo420/CyberArmor-Red-Team-Kit) | EIP-2612 permit phishing simulation — full attack chain from wallet connect to token drain |
| [Auth Recon Tagger](https://github.com/babakizo420/blessedops-auth-recon) | Classifies auth endpoints (login/reset/token/admin) from URL lists for targeted testing |
| [E-commerce Recon Template](https://github.com/babakizo420/ecom-recon-shopify-template) | Passive OSINT methodology for e-commerce attack surface mapping |

**Also building:**

| Project | What it does |
|---|---|
| [Pejji](https://pejji.com) | Security-first web agency — every site ships with CSP, HSTS, NDPA compliance, CI/CD security gates |

---

**Stack:**

`Python` `Bash` `JavaScript` `Solidity` `Burp Suite` `Nuclei` `FFUF` `Nmap` `Caido`

---

**Find me:**

[GitHub](https://github.com/babakizo420) · [YesWeHack](https://yeswehack.com) · [Upwork](https://www.upwork.com/freelancers/~01d1ef47c8b2a7a6aa)
